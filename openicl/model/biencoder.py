...
from transformers import AutoModel, PretrainedConfig, PreTrainedModel, AutoModelForCausalLM
...
class BiEncoderConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        self.order_training = order_training
...
class BiEncoder(PreTrainedModel):
    ...
    def __init__(self, config: BiEncoderConfig):
        ...
        self.scale_factor = torch.nn.Parameter(torch.tensor(config.scale_factor), requires_grad=True)
    ...
    def encode(self, input_ids, attention_mask, encode_ctx=False, **kwargs):
        if encode_ctx:
            if self.ctx_no_grad:
                with torch.no_grad():
                    enc_emb = self.ctx_model(input_ids=input_ids, attention_mask=attention_mask)
            else:
                enc_emb = self.ctx_model(input_ids=input_ids, attention_mask=attention_mask)
        else:
            enc_emb = self.question_model(input_ids=input_ids, attention_mask=attention_mask)
    ...
    def calc_nll_loss(
        ...
        if not self.pair_wise:
            ...
            # loss2 = partial_ranking_loss(scores[:, :hard_pos_neg_num], scores[:, hard_pos_neg_num:],
            #                              base_margin=self.margin*hard_pos_neg_num, margin=self.margin)
            # loss += loss2
    ...
    def calc_dpp_loss(
        ...
        # 1. directly batch compute costs too much memory， use for loop instead later
        # batch*batch*(num_pos_neg), num_ice
        # ctx_indices = ctx_indices.reshape(1, -1, num_ice).repeat(batch_size, 1, 1).reshape(-1, num_ice)
        # batch*batch*(num_pos_neg), num_all_ctx, num_all_ctx
        # kernel_matrix = kernel_matrix[:, None].repeat(1, batch_size * num_pos_neg, 1, 1).reshape(-1, num_all_ctx, num_all_ctx)
        # batch*batch*(num_pos_neg), num_ice, num_ice
        # neg_submatrix = indexing(kernel_matrix, ctx_indices)
        # batch, batch*(num_pos_neg)
        # scores = neg_submatrix.det().reshape(batch_size, -1)
        # ignore sign as PSD matrix det always >=0
        # scores = torch.linalg.slogdet(neg_submatrix).logabsdet.reshape(batch_size, -1)
        ...
        for i in range(batch_size):
            ...
            # per_sample_kernel_matrix = kernel_matrix
            # num_pos_neg, num_ice   ignore in-batch negative
            per_sample_ctx_indices = ctx_indices[i].reshape(-1, num_ice)

            # todo, bug while using infonce
            # neg_ctx_indices = torch.cat([ctx_indices[:i], ctx_indices[i+1:]], dim=0).reshape(-1, num_ice)
            # rand_indx = torch.randperm(len(neg_ctx_indices))
            # neg_ctx_indices = neg_ctx_indices[rand_indx][:in_batch_neg_num]
            # per_sample_ctx_indices = torch.cat([per_sample_ctx_indices, neg_ctx_indices], dim=0)
        ...
        if not self.pair_wise:
            ...
            # softmax_scores = F.normalize(scores, dim=-1).log()
        ...
        if loss == torch.nan or loss == torch.inf or loss == 0:
...
def ranking_loss(cand_scores, margin=0.):
    ...
    for i in range(1, n):
        ...
        # loss_func = torch.nn.MarginRankingLoss(margin)  # batch x i
        ones = torch.ones(pos_score.size(), device=pos_score.device)
        total_loss += loss_func(pos_score, neg_score, ones)
    return total_loss / n


def partial_ranking_loss(cand_scores, unranked_scores, base_margin=0.5, margin=0.1):
    batch, cand_n = cand_scores.size()
    unranked_cand_n = unranked_scores.size(1)
    total_loss = 0
    for i in range(1, cand_n + 1):
        pos_score = cand_scores[:, -i][:, None].repeat(1, unranked_cand_n)
        neg_score = unranked_scores
        loss_func = torch.nn.MarginRankingLoss(base_margin + margin * i)  # batch x i
        # loss_func = torch.nn.MarginRankingLoss(margin)  # batch x i
        ones = torch.ones(pos_score.size(), device=pos_score.device)
        total_loss += loss_func(pos_score, neg_score, ones)
    return total_loss / cand_n
